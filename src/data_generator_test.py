...
pb = data_generator.PandasBatches(
    ...
    batch_size=BATCH_SIZE, x_steps=X_STEPS, y_steps=Y_STEPS,
...
XAB = [
    ...
    18,19,20, 19,20,21, 20,21,22, 21,22,23, 22,23,24,
    23,24,25, 24,25,26, 25,26,27, 26,27,28, 27,28,29,
    28,29,30, 29,30,31, 30,31,32, 31,32,33, 32,33,34,
    33,34,35, 34,35,36, 35,36,37, 36,37,38, 37,38,39
]
...
YAB_STEPS = [
    ...
    19,20,21, 20,21,22, 21,22,23, 22,23,24, 23,24,25,
    24,25,26, 25,26,27, 26,27,28, 27,28,29, 28,29,30,
    29,30,31, 30,31,32, 31,32,33, 32,33,34, 33,34,35,
    34,35,36, 35,36,37, 36,37,38, 37,38,39, 38,39,40
]

Y_ALIGN_BATCHES_STEPS = np.array(YAB_STEPS).reshape(N_BATCHES, BATCH_SIZE, X_STEPS, 1)

YAB_1 = np.linspace(6, 40, 35)

Y_ALIGN_BATCHES_1 = YAB_1.reshape(N_BATCHES, BATCH_SIZE, Y_STEPS, 1)

Y_1_IDX = [3, 8, 13, 18, 23, 28, 33]



def make_aligned(y_steps=Y_STEPS):
    pb = data_generator.PandasBatches(
        data_frame=DFRAME, val_col="Vals", date_col="Dt",
        batch_size=BATCH_SIZE, x_steps=X_STEPS, y_steps=y_steps,
        align_size=True, noise_std=0)
...
def make_unaligned(y_steps=Y_STEPS):
    pb = data_generator.PandasBatches(
        data_frame=DFRAME, val_col="Vals", date_col="Dt",
        batch_size=BATCH_SIZE, x_steps=X_STEPS, y_steps=y_steps,
...
class BatchGenTest(unittest.TestCase):
    def test_aligned_data_size(self):
        ...
        self.assertEqual(pb.data[0], DFRAME.Vals[3])
        self.assertEqual(pb.dates[0], DFRAME.Dt[3])
    ...
    def test_y_iter(self):
        pb = make_aligned(y_steps=X_STEPS)
        cnt = 0
    ...
    def test_y_iter_1(self):
        pb = make_aligned(y_steps=Y_STEPS)
        cnt = 0
        idxs = []
